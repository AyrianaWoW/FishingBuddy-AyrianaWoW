...
GraphHandler.ClearPlot = function(frame)
   for _,elem in pairs({ GraphHandler.BAR, GraphHandler.LEGEND }) do
      for idx,what in pairs(frame.elements[elem]) do
	 what:Hide();
      end
   end
end
...
GraphHandler.PlotGrid = function(frame, label, texth, width, textv, height)
   ...
   if ( texth ) then
      local linesh = 0;
      for _,_ in pairs(texth) do
	 linesh = linesh + 1;
      end
      local offset = height/(linesh-1);
      for label,tick in pairs(texth) do
   ...
   if ( textv ) then
      local linesv = 0;
      for _,_ in pairs(textv) do
	 linesv = linesv + 1;
      end
      local offset = frame.barSpacing + frame.barWidth;
      for label,tick in pairs(textv) do
	 local line = frame.elements[GraphHandler.LINE][ldx];
	 if ( line ) then
